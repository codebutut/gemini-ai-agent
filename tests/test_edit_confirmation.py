...
class TestEditConfirmation(unittest.TestCase):
    def setUp(self):
        ...
        self.executor = ToolExecutor(self.status_cb, self.terminal_cb, self.confirm_cb)
    ...
    def test_tool_executor_uses_modified_args(self):
        ...
        with patch.dict(
            "gemini_agent.core.tools.TOOL_FUNCTIONS", {tool_name: MagicMock(return_value="Output")}
        ) as mock_tools:
            # Act
