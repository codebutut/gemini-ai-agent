...
from gemini_agent.utils.logger import get_logger, AgentLoggerAdapter
...
class GeminiWorker(QObject):
    ...
    def __init__(self, config: WorkerConfig):
        ...
        self._confirmation_modified_args: Optional[Dict[str, Any]] = None
    ...
    def confirm_tool(self, confirmation_id: str, allowed: bool, modified_args: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self._current_confirmation_id == confirmation_id:
            ...
            if self._loop and self._loop.is_running():
                ...
                self.log.warning("Worker loop not available/running to set confirmation event. Falling back to direct set.")
    ...
    async def _request_tool_confirmation(self, fn_name: str, fn_args: Dict[str, Any]) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    def _create_config(self, tools_config: Optional[types.Tool] = None) -> types.GenerateContentConfig:
        config_args = {
    ...
    async def run_async(self) -> None:
        ...
        try:
            ...
            def sync_confirmation_callback(fn_name: str, fn_args: Dict[str, Any]) -> Tuple[bool, Optional[Dict[str, Any]]]:
                future = asyncio.run_coroutine_threadsafe(
                    self._request_tool_confirmation(fn_name, fn_args),
                    loop
                )
            ...
            self.tool_executor = ToolExecutor(
                ...
                plugin_manager=self.config.plugin_manager
            )
    ...
    async def _run_grounding_mode(self, client: genai.Client, gemini_contents: List[types.Content]) -> None:
        ...
        await GeminiWorker.RATE_LIMITER.acquire_async()
    ...
    async def _handle_function_calls(self, client: genai.Client, gemini_contents: List[types.Content]) -> None:
        ...
        extra_tools = self.config.plugin_manager.get_all_tools() if self.config.plugin_manager else []
