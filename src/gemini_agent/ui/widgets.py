...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QApplication, QPushButton, QSizePolicy, QCompleter
)
from PyQt6.QtCore import Qt, QTimer, QSize, pyqtSignal, QStringListModel
from PyQt6.QtGui import (
    QDesktopServices, QFontDatabase, QFont, QSyntaxHighlighter, 
    QTextCharFormat, QColor, QTextDocument, QTextCursor
)
...
class MessageBubble(QFrame):
    ...
    def initUI(self):
        ...
        layout = QVBoxLayout(self)
        layout.setContentsMargins(15 if self.is_user else 0, 10 if self.is_user else 0, 15 if self.is_user else 0, 10 if self.is_user else 0)
        layout.setSpacing(5)
    ...
    def _add_markdown_text(self, text: str, layout: QVBoxLayout):
        ...
        lbl.setStyleSheet(f"color: {text_color}; border: none; font-size: 16px;")
    ...
    def _markdown_to_html(self, text: str) -> str:
        ...
        try:
            ...
            if "<table>" in html:
                ...
                html = html.replace("<table>", f'<table border="1" cellspacing="0" cellpadding="5" width="100%" style="border-collapse: collapse; border-color: {border_color};">')
...
class AutoResizingTextEdit(QTextEdit):
    ...
    def _setup_completer(self):
        ...
        keywords = [
            "/clear", "/help", "/reset", "/conductor", 
            "analyze", "refactor", "explain", "fix", "search"
        ]
    ...
    def keyPressEvent(self, event):
        ...
        if (not is_shortcut and (len(completion_prefix) < 1 or event.text() == "")):
            if self._completer: self._completer.popup().hide()
            return
